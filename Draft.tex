...
\begin{align*}
    \implies & g_1(m, n) = \pmb{L} [f_1(m, n)] \land g_2(m, n) = \pmb{L} [f_2(m, n)] \\
    \implies & a \cdot g_1(m, n) + b \cdot g_2(m, n) = \pmb{L} [a \cdot f_1(m, n) + b \cdot f_2(m, n)]
\end{align*}
...
\begin{align*}
    \implies & g(m, n) = \pmb{L} [f(m, n)]\\
    \implies & g(m - p, n - q) =  \pmb{L} [f(m - p, n - q)]
...
of these two properties is that the function has to behave the same throughout the entire picture.\smallskip
...
Also notated as $K=vh^{\pmb{T}}$, where $\pmb{T}$ denotes tranposing $h$.

A method to check whether a given Kernel is separable is to use \emph{Singular Value Decomposition (SVD)}\cite{Szeliski_2022}

\[        
    \pmb{K} = \sum_{i}{\sigma_i \cdot u_i \cdot v_i^{\pmb{T}}} 
\]
or more generally
\begin{align*}
        \pmb{K} =& \textbf{U}_{m\times p} \pmb{\textstyle\sum}_{p\times p} \textbf{V}_{p\times n}^{\pmb{T}} \\
        \pmb{K} =& \begin{bmatrix}
            u_0 & \hdots & u_{p-1}
        \end{bmatrix}
        \begin{bmatrix}
...
Note that for Corner filtering, the kernel is not normalized.
...
\section{Simple examples of Linear Filters}
One notable example of a basic filter is the box filter, where each pixel is determined by the average of a $K \times K$ 
...
old pixel and adding the newest pixels to the running sum.
